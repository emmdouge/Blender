...
ffmpeg -i %1_frames\output_true12fps.mp4 -r 24 %1_frames\output_true24fps.mp4 
ffmpeg -i %1_frames\output_true12fps.mp4 -vf negate %1_frames\outputN.mp4 
ffmpeg -i %1_frames\output_true12fps.mp4 -i %1_frames\outputN.mp4 -filter_complex "[0:v]scale=1280x720[bottom];[1:v]scale=1280x720[top];[bottom][top]overlay=enable='if(gt(n,0),not(mod(n\,12)),0)'" %1_frames\output24b.mp4
...
ffmpeg -r 6 -f image2 -s 1920x1080 -i %1_frames\in%%d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p %1_frames\testb.mp4
