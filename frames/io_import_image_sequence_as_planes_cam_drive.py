...
class IMPORT_OT_image_to_plane(Operator, AddObjectHelper):
    ...
    def add_driver(self, source, target, prop, dataPath, index, func = ''):
        ...
        if index != -1:
            ...
            d = source.driver_add(prop)[0].driver
        ...
        me = source.data # mesh
        #mid = sqrt(pow(mmx-camloc.x,2)+pow(mmy-camloc.y,2)+pow(mmz-camloc.y,2))
        mid = midpointXY((mw * me.vertices[0].co),(mw * me.vertices[2].co))
        dist = mid - camloc
        
        d.expression = func + "(" + v.name + "/" + str(dist.y) + ")"
